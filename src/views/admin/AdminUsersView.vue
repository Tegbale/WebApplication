<template>
  <div>
    ...
    <div v-if="loadingData" class="flex justify-center items-center h-72">
      <svg
        aria-hidden="true"
        class="inline w-16 h-16 mr-2 text-white animate-spin fill-blue-600"
        viewBox="0 0 100 101"
        fill="none"
    ...
    <div v-else>
      <BaseDataTable class="hidden md:block">
        ...
        <template #table-body>
          <template v-if="usersStore && usersStore.users.data.length > 0">
            <tr
              class="border-b-2 border-gray-100"
              v-for="(user, i) in usersStore.allUsers.data"
              :key="i"
            >
      ...
      <template v-if="usersStore && usersStore.users.data.length > 0">
        ...
        >
          <BaseMobileDataTable
            :column-one-text="user ? user.fullname : ''"
            :column-two-text="user ? user.email : ''"
            :column-three-text="user ? user.role : ''"
    ...
    <div class="flex justify-center items-center py-4 mt-4">
      <v-pagination
        ...
        :range-size="1"
        active-color="#408ed5"
        @update:modelValue="fetchAllUsers"
      />
    </div>
  </div>
      ...
      <form>
        ...
        <div class="grid grid-auto-fit gap-3">
          <div>
            ...
            />
            <span
              v-if="errors && errors.firstname && errors.firstname.length > 0"
          ...
          <div v-if="modalTitle == 'Create New User'">
            ...
            <span
              v-if="errors && errors.password && errors.password.length > 0"
              class="text-red-500 text-sm pl-4"
            >
              {{ errors.password[0] }}
            </span>
        ...
        <div class="pt-5">
          ...
          <div class="grid grid-auto-fit gap-3">
            ...
            <div>
              ...
              <span
                v-if="errors && errors.role.length > 0"
                class="text-red-500 text-sm pl-4"
              >
                {{ errors.role[0] }}
              </span>
...
const saveUser = async () => {
  ...
  if (modalTitle.value === "Create New User") {
    ...
    try {
      ...
      await usersStore.getAllUsers(currentPage.value);
    }
...
const fetchAllUsers = async () => {
  try {
    const { data } = await usersStore.getAllUsers(currentPage.value);
...
onMounted(async () => {
  ...
  try {
    let { data } = await usersStore.getAllUsers(currentPage.value);
