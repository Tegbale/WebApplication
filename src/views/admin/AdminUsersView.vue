  ...
  <div>
    ...
    <div v-else>
      <BaseDataTable class="hidden md:block">
        ...
        <template #table-body>
          <template v-if="usersStore && usersStore.users && usersStore.users.data.length > 0">
            <tr class="border-b-2 border-gray-100" v-for="(user, i) in usersStore.allUsers.data" :key="i">
      ...
      <!-- Mobile design  -->
      <template v-if="usersStore && usersStore.users &&  usersStore.users.data.length > 0">
        <div class="grid grid-auto-fit gap-4 md:hidden pt-4" v-for="(user, i) in usersStore.allUsers.data" :key="i">
    ...
    <div class="flex justify-center items-center py-4 mt-4">
      <!-- <v-pagination v-model="currentPage" :pages="pageCount" :range-size="1" active-color="#408ed5"
