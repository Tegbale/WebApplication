<template>
    <div>
      <div>
        <div class="flex justify-between items-end space-x-2 pt-20">
          <BasePageTitle pageTitle="Guardians" :showBtn="true" btnText="Add Guardian" @clicked="handleModalAction" />
        ...
        <div v-else>
          <BaseDataTable class="hidden md:block">
            ...
            <template #table-body>
              <template v-if="GuardianLists && GuardianLists.length > 0">
                <tr class="border-b-2 border-gray-100 hover:bg-gray-50" v-for="(teacherItem, i) in GuardianLists"
                  :key="teacherItem.id">
                  ...
                  <td class="flex justify-end p-3 space-x-2">
                    <button @click="handleViewGuardian(teacherItem.id)"
                    ...
                    <button @click="handleEditGuardian(teacherItem.id)"
                    ...
                    <button @click="handleDeleteGuardian(teacherItem.id)"
          ...
          <!-- Mobile design  -->
          <template v-if="GuardianLists && GuardianLists.length > 0">
            <div class="grid grid-auto-fit gap-4 md:hidden pt-4" v-for="teacherItem in GuardianLists" :key="teacherItem.id">
              <BaseMobileDataTable :column-one-text="teacherItem.name" :column-two-text="teacherItem.full_name"
  ...
  const teacherStore = useGuardian();
  ...
  const handleEditGuardian = async (id) => {
    ...
    try {
      ...
      // teacherData.headGuardian = data.location;
  ...
  const fetchAllGuardians = async () => {
    ...
    try {
      const res = await teacherStore.fetchAllGuardians();
      GuardianLists.value = res.data.data;
  ...
  const handleViewGuardian = async (id) => {
    isEditing.value = false;
    isCreating.value = false;
    ...
    try {
      const { data } = await teacherStore.fetchAGuardianByID(id);
      ...
      toastStore.showToast({
        title: "Ooops!",
        message: "Unable to Fetch Guardian details",
        type: "error",
        timeout: 4000,
      });
    }
  ...
  const dataExport = (value) => {
    ...
    switch (value) {
      case "pdf":
        ...
        break;
      case "csv":
        dataParser().exportDataFromJSON(GuardianLists.value, docName, value);
  
        break;
      default:
        // code block
        dataParser().exportDataFromJSON(GuardianLists.value, null, null);
    }
  };
  ...
  const rowArray = computed(() => {
    // remove the created_at key from each object in the GuardianLists array
    let newArray = GuardianLists.value.map(({ created_at, ...rest }) => {
      return rest;
    });
