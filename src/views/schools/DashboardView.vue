...
import BaseSinglePost from "@/components/BaseComponents/BaseSinglePost.vue";
import { onMounted, ref } from "vue";
import { useSchDashboard } from "@/stores/schoolDashboard";
const dashboardStore = useSchDashboard();

const totalClassrooms = ref(null);
...
onMounted(async () => {
  ...
  try {
    let { data } = await dashboardStore.fetchDashboardMetrics();
    console.log(JSON.stringify(data, null, 2));
    totalParents.value = data.total_parents;
...
<template>
  <div>
    ...
    <div class="pt-10 grid grid-auto-fit-sm gap-8">
      <DashboardCards
        :cardValue="totalClassrooms"
        cardTitle="classrooms"
        class="relative"
      >
        <template #icon>
          <div
    ...
    <div class="pt-10 md:pr-4 w-full md:max-w-[50%] h-[70vh]">
      <div class="w-full py-6 bg-white rounded-lg">
        <p class="px-4 font-roboto text-base sm:text-xl text-tegbale-blue">
          Latest Posts
        </p>
        <div
          class="space-y-3 scroll_container h-80 overflow-y-auto"
          v-if="!posts"
        >
          <div class="pt-6 px-4 border-b">
            <BaseSinglePost />
          </div>
        ...
        <div class="px-4 py-4 text-base text-tegbale-text-gray" v-else>
          ...
          <p>There are no Post yet. Latest posts will be displayed here</p>
        </div>
      </div>
    </div>
  </div>
